...
[![Coverage](https://img.shields.io/badge/coverage-79.2%25-brightgreen)](https://github.com/serblowup/MicroPKI)
...
- SQLite БД для хранения сертификатов **промежуточных УЦ и конечных субъектов** (корневой сертификат хранится только в файловой системе)
- Уникальные серийные номера (64-битные: timestamp + random)
- Автоматическое сохранение сертификатов в БД при выпуске (intermediate и leaf)
...
| **SQLite Database** | Хранение сертификатов **промежуточных УЦ и конечных субъектов**, метаданных CRL и скомпрометированных ключей. **Корневой сертификат хранится только в файловой системе.** |
| **File System** | Хранение зашифрованных ключей УЦ, PEM-сертификатов (включая корневой) и CRL-файлов с правами 0600/0700 |
...
# Корневой сертификат сохраняется ТОЛЬКО в файловой системе (не в БД)
...
**Примечание:** Команда `list-certs` показывает только сертификаты, сохранённые в БД (промежуточные УЦ и конечные). Корневой сертификат в этот список **не включается**, так как он хранится только в файловой системе.
...
**Примечание:** Команда `show-cert` ищет сертификат только в БД. Для получения корневого сертификата используйте `GET /ca/root` или直接 читайте файл `certs/ca.cert.pem`.
...
> **Примечание:** Этот эндпоинт ищет сертификаты **только в БД**. Корневой сертификат (который не сохраняется в БД) через этот эндпоинт недоступен. Для получения корневого сертификата используйте `GET /ca/root`.
...
# 3. Создание корневого УЦ (сертификат НЕ сохраняется в БД)
./bin/micropki ca init \
    --subject "/CN=Test Root CA" \
    --passphrase-file ./root.pass \
    --out-dir ./pki \
...
# 4. Создание промежуточного УЦ (сертификат сохраняется в БД)
./bin/micropki ca issue-intermediate \
    --root-cert ./pki/certs/ca.cert.pem \
    --root-key ./pki/private/ca.key.pem \
    --root-pass-file ./root.pass \
...
6. **База данных**: SQLite с правами 0644, содержит **только сертификаты промежуточных УЦ и конечных субъектов**. **Корневой сертификат хранится только в файловой системе** и не дублируется в БД.
