...
| **STR-13** | Добавлены модули для CRL и отзыва | **internal/crl/**: `crl.go`, `manager.go`<br>**internal/revocation/**: `revocation.go` |
| **STR-14** | README обновлен с инструкциями | **README.md**: добавлены разделы с командами `revoke`, `gen-crl`, `check-revoked`, примерами `curl` для CRL, проверкой через OpenSSL |
| **STR-15** | Структура директорий расширена для CRL | **cmd/micropki/main.go**: функция `runCAGenCRL` создает директорию `crl/` и сохраняет файлы `<ca>.crl.pem` |

## 2. Команды CLI

| ID | Требование | Где реализовано |
|-----|------------|-----------------|
| **CLI-18** | `ca revoke` с поддержкой причин | **cmd/micropki/main.go**: функция `runCARevoke()`<br>**internal/revocation/revocation.go**: `ReasonCodeToInt()` и `RevokeCertificate()` |
| **CLI-19** | `ca gen-crl` с параметрами | **cmd/micropki/main.go**: функция `runCAGenCRL()` |
| **CLI-20** | `ca check-revoked` проверка статуса | **cmd/micropki/main.go**: функция `runCACheckRevoked()`<br>**internal/revocation/revocation.go**: `CheckRevoked()` |
| **CLI-21** | Существующие команды не изменены | Команды `ca issue-cert` и `ca issue-intermediate` остались без изменений |
...
| ID | Требование | Где реализовано |
|-----|------------|-----------------|
| **CRL-1** | CRLv2 формат с полями | **internal/crl/crl.go**: `GenerateCRL()` - использует `x509.CreateRevocationList` с заполнением Number, ThisUpdate, NextUpdate, Issuer, AuthorityKeyId |
| **CRL-2** | Расширения CRL (AKI, CRL Number, Reason Code) | **internal/crl/crl.go**: `GenerateCRL()` - AKI из сертификата, Number, Reason Code как расширение |
| **CRL-3** | Интеграция с БД при отзыве | **internal/revocation/revocation.go**: `RevokeCertificate()` обновляет статус, дату и причину<br>**internal/database/certificates.go**: `UpdateCertificateStatus()` |
| **CRL-4** | Подпись CRL ключом УЦ | **internal/crl/crl.go**: `GenerateCRL()` - `x509.CreateRevocationList` подписывает CRL |
| **CRL-5** | Хранение CRL в поддиректории | **cmd/micropki/main.go**: создает директорию `crl` и сохраняет файлы в `<out-dir>/crl/<ca>.crl.pem` |
| **CRL-6** | Сохранение номера CRL | **internal/crl/manager.go**: `GetNextCRLNumber()` и `UpdateCRLMetadata()` используют таблицу `crl_metadata` |
| **CRL-7** | Поддержка всех кодов причин | **internal/revocation/revocation.go**: `ReasonCodeToInt()` и `ReasonCodeToString()` поддерживают все 10 кодов RFC 5280 |
| **CRL-8** | Delta-CRL не требуется | Не реализовано, что соответствует требованию |
...
| ID | Требование | Где реализовано |
|-----|------------|-----------------|
| **REPO-9** | GET /crl с параметром ca | **internal/repository/handlers.go**: `handleCRL()` - обрабатывает `?ca=root|intermediate`, возвращает `Content-Type: application/pkix-crl` |
| **REPO-10** | GET /crl/{filename} альтернативный путь | **internal/repository/handlers.go**: `handleCRLFile()` - обслуживает `/crl/root.crl.pem` и `/crl/intermediate.crl.pem` |
| **REPO-11** | Заголовки кэширования | **internal/repository/handlers.go**: `serveCRLFile()` - добавляет `Last-Modified`, `ETag`, `Cache-Control: max-age=3600` |
| **REPO-12** | Логирование CRL запросов | **internal/repository/middleware.go**: `withLogging()` - логирует все HTTP запросы, включая CRL |

## 5. База данных и обновления схемы

| ID | Требование | Где реализовано |
|-----|------------|-----------------|
| **DB-5** | Использование существующих полей | **internal/database/certificates.go**: поля `status`, `revocation_reason`, `revocation_date` обновляются через `UpdateCertificateStatus()` |
| **DB-6** | Новая таблица crl_metadata | **internal/database/schema.go**: `GetSchema()` создает таблицу `crl_metadata` с полями `ca_subject`, `crl_number`, `last_generated`, `next_update`, `crl_path` |
| **DB-7** | Миграции для существующих БД | **internal/database/schema.go**: `GetMigrations()` включает миграцию версии 4 для создания `crl_metadata`<br>`ApplyMigrations()` применяет миграции при запуске |

## 6. Логирование и аудит

| ID | Требование | Где реализовано |
|-----|------------|-----------------|
| **LOG-9** | Логи событий отзыва | **internal/revocation/revocation.go**: `RevokeCertificate()` - логирование успеха, предупреждение для уже отозванных<br>**cmd/micropki/main.go**: `runCARevoke()` - логирование ошибок |
| **LOG-10** | Логи генерации CRL | **cmd/micropki/main.go**: `runCAGenCRL()` - логирование начала, успешного завершения, ошибок |
| **LOG-11** | JSON логи аудита | **internal/logger/logger.go**: `AuditJSON()` - используется для записи отзывов и генерации CRL |

## 7. Тестирование

| ID | Требование | Где реализовано |
|-----|------------|-----------------|
| **TEST-21** | Жизненный цикл отзыва | **tests/integration_test.go**: `TestRevocationLifecycle()` - полный цикл: выпуск → проверка → отзыв → проверка БД → генерация CRL → проверка содержимого |
| **TEST-22** | Проверка подписи CRL через OpenSSL | **scripts/test-revocation-with-openssl.sh**: использует `openssl crl -CAfile` для проверки подписи (запускается через `make scripts`) |
| **TEST-23** | Увеличение номера CRL | **tests/integration_test.go**: `TestRevocationLifecycle()` проверяет генерацию CRL с номером 1<br>**internal/crl/manager.go**: `GetNextCRLNumber()` реализует инкремент |
| **TEST-24** | Отзыв несуществующего сертификата | **tests/revocation_test.go**: `TestRevokeNonExistentCertificate()` |
| **TEST-25** | Отзыв уже отозванного сертификата | **tests/revocation_test.go**: `TestRevokeAlreadyRevokedCertificate()` |
| **TEST-26** | Распространение CRL через HTTP | **tests/repository_test.go**: `TestCRLEndpoint()` - проверяет все эндпоинты CRL |
| **TEST-27** | Интероперабельность с OpenSSL s_client | **scripts/test-revocation-with-openssl.sh**: запускает HTTPS сервер и проверяет отказ при подключении с отозванным сертификатом (запускается через `make scripts`) |

## Ключевые изменения в `main.go` для 4 спринта

| Команда | Назначение |
|---------|------------|
| `ca revoke` | Отзыв сертификата по серийному номеру |
| `ca gen-crl` | Генерация CRL для указанного УЦ |
| `ca check-revoked` | Проверка статуса отзыва сертификата |
| Флаг `--reason` | Код причины отзыва |
| Флаг `--crl` | Путь к CRL файлу для обновления |
| Флаг `--ca` | Имя УЦ (root/intermediate) |
| Флаг `--next-update` | Дней до следующего обновления CRL |

## Сводная таблица по новым файлам

| Файл | Какие требования реализованы |
|------|---------------------------|
| `internal/crl/crl.go` | CRL-1, CRL-2, CRL-4, CRL-5 |
| `internal/crl/manager.go` | CRL-6, DB-6 |
| `internal/revocation/revocation.go` | CLI-18, CLI-20, CRL-3, CRL-7, LOG-9, TEST-24, TEST-25 |
| `tests/crl_test.go` | CRL-1, CRL-2, CRL-4, CRL-6 |
| `tests/revocation_test.go` | TEST-24, TEST-25 |
| `scripts/test-revocation-with-openssl.sh` | TEST-22, TEST-27 |
...
| `internal/logger/logger.go` | Метод AuditJSON уже существовал | LOG-11 |
| `README.md` | Добавлены инструкции по отзыву и CRL | STR-14 |
| `Makefile` | Добавлены цели revoke, gen-crl, check-revoked, test-revocation | STR-14, TEST-22, TEST-27 |
